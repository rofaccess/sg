...
class OrdenesCompraController < ApplicationController
  ...
  def generar_ordenes_multiples_pedidos(pedidos_ids)
    ordenes_cont = 0
    pedidos_ids.each do |pedido_id|
      ...
      cotizaciones.each do |c|
        ...
        c.pedido_cotizacion_detalles.each do |d|
          if mejores_precios[d.pedido_compra_detalle_id][0] == d.id
            ...
            total_requerido += d.cantidad_cotizada*d.costo_unitario
            componentes.push(d.componente_id)
          end
        end
  ...
  def orden_personalizado
    @search =OrdenCompra.search(params[:q])
    unless params[:pedido_compra_ids].nil?
      @ordenes_cont = generar_ordenes_multiples_pedidos(params[:pedido_compra_ids])
    else
      @ordenes_cont = 1
    ...
    flash.notice = "Se han generado #{@ordenes_cont} orden#{@ordenes_cont > 1 ? 'es' : ''} de compra."
  end
